...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // agora pega os dados da ONG
    const ongInfo = usuario.info;
    const ongId = ongInfo.ong_id; // ✅ ID correto
    if (!ongId) {
    ...
    async function carregarDadosDaOng() {
        try {
            ...
            if (nomeSpans.length)
                nomeSpans.forEach((el) => (el.textContent = ong.nome || "—"));
            ...
            alert(
                "Não foi possível carregar os dados da ONG. Verifique a conexão com o servidor."
            );
        }
    }
    ...
    async function salvarEdicao() {
        ...
        try {
            ...
            if (nomeSpans.length)
                nomeSpans.forEach((el) => (el.textContent = updated.nome || nome));
    ...
    if (btnEditarDados) {
        ...
        const fallback = Array.from(document.querySelectorAll("button")).find((b) =>
            /editar\s+dados/i.test(b.textContent)
        );
        if (fallback)
